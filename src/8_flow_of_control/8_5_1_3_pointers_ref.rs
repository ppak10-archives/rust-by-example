...
fn main() {
  ...
  // What if you don't start with a reference? `reference` was a `&` because the
  // right side was already a reference. This is not a reference because the
  // right side is not one.
  let _not_a_reference = 3;

  // Rust provides `ref` for exactly this purpose. It modifies the assignment so
  // that a reference is created for the element; this reference is assigned.
