 ...
 * any consequences? It sure does. Observe how using a closure as a function
...
// `F` must implement `Fn` for a closure which takes no inputs and returns
// nothing - exactly what is required for `print`.
fn apply<F>(f: F) where
  F: Fn() {
    f();
}
