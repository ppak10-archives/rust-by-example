 ...
 * ```
 *
 * For the rest of this chapter, lets assume we are making a binary, rather than
 ...
 * ```
 *
 * The `main.rs` is the root source file for your new project -- nothing new
 * there. The `Cargo.toml` is the config file for `cargo` for this project
 * (`foo`). If you look inside it, you should see something like this:
 *
 * ```
 * [package]
 * name = "foo"
 ...
 * find lots of greate packages on crates.io (the official Rust package
 * registry). One popular choice is clap. As of this writing, the most recent
